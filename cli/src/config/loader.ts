...
import { type OuroborosConfig, OuroborosConfigSchema } from "./types.ts";

export const OUROBOROS_DIR = ".ouroboros";
export const CONFIG_FILE = "config.yaml";
export const PROGRESS_FILE = "progress.txt";

/**
 * Get the full path to the ouroboros directory
 */
export function getOuroborosDir(workDir = process.cwd()): string {
	return join(workDir, OUROBOROS_DIR);
}
...
export function getConfigPath(workDir = process.cwd()): string {
	return join(workDir, OUROBOROS_DIR, CONFIG_FILE);
}
...
export function getProgressPath(workDir = process.cwd()): string {
	return join(workDir, OUROBOROS_DIR, PROGRESS_FILE);
}

/**
 * Check if ouroboros is initialized in the directory
 */
...
/**
 * Load the ouroboros config from disk
 */
export function loadConfig(workDir = process.cwd()): OuroborosConfig | null {
	...
	try {
		...
		return OuroborosConfigSchema.parse(parsed);
	} catch (error) {
		// Return default config if parsing fails
		return OuroborosConfigSchema.parse({});
	}
}
